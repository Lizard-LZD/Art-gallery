window.onload = () => {
  ...
  var buttons = document.querySelectorAll("carousel-buttons");
  ...
  function initialize() {
    ...
    text.innerHTML=text.textContent.replace(/\S/g,"<span-text>$&</span-text>")
    // 定义几秒后恢复
    let goBackTime=2;

    const letters = document.querySelectorAll("span-text");
    for(let i=0;i<letters.length;i++){
        letters[i].addEventListener('mouseover',function(){
            letters[i].classList.add('active')
            letters[i].classList.add('active'+i/*'have-been-active'+i*/)
            // 恢复,用lambda表达式
            setTimeout(()=>{
                let letter = document.querySelector('.active'+i/*'.have-been-active'+i*/)
